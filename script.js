var APPS={'cine':{title:'CINE // HUB',path:'script/Apps/Cine/index.html',icon:'https://cdn.worldvectorlogo.com/logos/netflix-logo-icon.svg',pinned:true},'term':{title:'Spotify',path:'script/Apps/Spotify/index.html',icon:'https://cdn.pixabay.com/photo/2016/10/22/00/15/spotify-1759471_1280.jpg',pinned:true},'files':{title:'PS5 Emu',path:'script/Apps/Ps5/index.html',icon:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ-OeL_be7RFaoHi3PswkuAR5XcMgBNRDynsg&s',pinned:true},'web':{title:'Cine-Web',path:'script/Apps/Web/index.html',icon:'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSeD89ZcX5W1FBtal7RerasT27q-OmZqnBixQ&s',pinned:true},'settings':{title:'CONFIG',internal:true,icon:'https://cdn.iconscout.com/icon/free/png-256/free-apple-settings-icon-svg-download-png-493162.png',pinned:true},'discord':{title:'Discord',path:'https://discord.com/app',icon:'https://assets-global.website-files.com/6257adef93867e50d84d30e2/636e0a6a49cf127bf92de1e2_icon_clyde_blurple_RGB.png',pinned:false},'ciri':{title:'CIRI',path:'CIRI.html',icon:'https://cdn-icons-png.flaticon.com/512/8682/8682056.png',pinned:true}};
let wallpaperRegistry={"Default":{id:"Default",name:"Snake Skeleton",url:"Videos/default.mp4",locked:false},"green":{id:"green",name:"Green Anime",url:"Videos/green.mp4",locked:false},"#33A56":{id:"hunt_trait",name:"Hunt Showdown",url:"Videos/33A56.mp4",locked:true},"#Minecraft01":{id:"Tree",name:"Tree Of Life",url:"Videos/Minecraft01.mp4",locked:true},"#45E33":{id:"SnowFall",name:"Snow Fall",url:"Videos/45E33.mp4",locked:true},"#Minecraft02":{id:"Minecraft02",name:"Sakura Pond",url:"Videos/Minecraft02.mp4",locked:true},"#Minecraft03":{id:"Minecraft03",name:"Minecraft Paradise",url:"Videos/Minecraft03.mp4",locked:true}};
var sysConfig=JSON.parse(localStorage.getItem('cine_sys_config'))||{optBg:false,shortBoot:false,panicKey:'`',homeWallpaper:'Default',lockWallpaper:'green',cloak:'none',wpLoop:false,idleLock:false,redirPrompt:false};
if(sysConfig.wpLoop===undefined)sysConfig.wpLoop=false;if(sysConfig.idleLock===undefined)sysConfig.idleLock=false;if(sysConfig.redirPrompt===undefined)sysConfig.redirPrompt=false;
...
window.updateSysSetting=function(k,v){sysConfig[k]=v;localStorage.setItem('cine_sys_config',JSON.stringify(sysConfig));if(k==='optBg'||k==='wpLoop')updateWallpaperLoop();if(k==='redirPrompt')setupRedirectionPrompts();};
...
let keyHistory=[];const secretCombo=['ArrowUp','ArrowDown','ArrowUp','ArrowDown'];
document.addEventListener('keydown',function(event){
 if(document.getElementById('fake-block-screen').classList.contains('hidden-block')){
  if(bootActive&&event.key==='Enter'&&document.getElementById('boot-layer').style.display!=='none'){enterCount++;if(enterCount>=2)skipBootSequence();setTimeout(()=>{enterCount=0;},500);}
  if(event.key&&sysConfig.panicKey&&event.key.toLowerCase()===sysConfig.panicKey.toLowerCase()){window.location.href="https://google.com";}
 }else{
  keyHistory.push(event.key);if(keyHistory.length>secretCombo.length)keyHistory.shift();
  if(JSON.stringify(keyHistory)===JSON.stringify(secretCombo)){
   document.getElementById('fake-block-screen').classList.add('hidden-block');
...
document.addEventListener("DOMContentLoaded",()=>{applyCloak();loadDynamicResources();loadDesktop();setupRedirectionPrompts();});
...
function updateWallpaperLoop(){var bgV=document.getElementById('bg-video');var lV=document.getElementById('lock-video');if(bgV)bgV.loop=sysConfig.wpLoop;if(lV)lV.loop=sysConfig.wpLoop;if(sysConfig.optBg){if(bgV)bgV.pause();}else if(isDesktopActive&&bgV&&bgV.paused){bgV.play().catch(e=>{});}}
...
function loadDesktop(){document.querySelectorAll('.desktop-app').forEach(e=>e.remove());desktopLayout.forEach((i,idx)=>{let d=document.createElement('div');d.className='desktop-app';d.style.left=i.x+'px';d.style.top=i.y+'px';d.dataset.idx=idx;if(i.type==='folder'){let g=`<div class="d-folder-grid">`;i.apps.slice(0,4).forEach(a=>{if(APPS[a])g+=`<img src="${APPS[a].icon}">`;});g+=`</div><div class="d-label">Folder</div>`;d.innerHTML=g;d.onclick=()=>{openFolderView(idx)};}else{let a=APPS[i.id];if(a){d.innerHTML=`<img src="${a.icon}" class="d-icon"><div class="d-label">${a.title}</div>`;d.onclick=()=>toggleApp(i.id);}}
d.onmousedown=(ev)=>{ev.stopPropagation();let moved=false;let sx=ev.clientX,sy=ev.clientY;const mm=(em)=>{if(Math.abs(em.clientX-sx)>3||Math.abs(em.clientY-sy)>3){moved=true;DragSystem.start(em,d,'desktop',idx);d.removeEventListener('mousemove',mm);}};d.addEventListener('mousemove',mm);d.addEventListener('mouseup',()=>{d.removeEventListener('mousemove',mm);},{once:true});};document.getElementById('desktop-area').appendChild(d);});}
function saveDesktop(){localStorage.setItem('cine_desktop_v2',JSON.stringify(desktopLayout));loadDesktop();}

function openFolderView(idx){
 const fOverlay=document.getElementById('folder-view-overlay');const fBox=document.getElementById('folder-view-box');fBox.innerHTML='';
 let fData=desktopLayout[idx];if(fData&&fData.type==='folder'){
  fData.apps.forEach(aId=>{let app=APPS[aId];if(app){let ai=document.createElement('div');ai.className='folder-item';ai.innerHTML=`<img src="${app.icon}"><span>${app.title}</span>`;ai.onclick=(e)=>{e.stopPropagation();toggleApp(aId);closeFolderView();};fBox.appendChild(ai);}});
  fOverlay.style.display='flex';setTimeout(()=>fOverlay.classList.add('open'),10);
 }
}
window.closeFolderView=function(e){const fOverlay=document.getElementById('folder-view-overlay');if(e&&e.target.closest('#folder-view-box'))return;fOverlay.classList.remove('open');setTimeout(()=>fOverlay.style.display='none',300);}
...
end(e){if(!this.sourceEl)return;if(!this.dragging){this.reset();return;}
...
// REDIRECTION PROMPTS (DEFENDS VS GOGUARDIAN)
function setupRedirectionPrompts(){
 if(sysConfig.redirPrompt){
  window.addEventListener('beforeunload',(e)=>{e.preventDefault();e.returnValue='Redirection Warning';return 'Redirection Warning';});
  document.addEventListener('click',(e)=>{let a=e.target.closest('a');if(a&&a.href&&!a.href.startsWith('javascript:')){e.preventDefault();if(confirm(`Leaving CINE-OS to: \n\n${a.href}\n\nDo you want to proceed?`)){window.location.href=a.href;}}});
 }else{
  window.onbeforeunload=null;
 }
}

let activeMedia=null;let notiHideTimeout;const cineNoti=document.getElementById('cine-noti');
...
setInterval(()=>{
 ...
 if(found!==activeMedia){if(found){activeMedia=found;setupMediaListeners();showNoti();}else{activeMedia=null;hideNoti();}}
 if(activeMedia){
...
function setupMediaListeners(){
 if(!activeMedia)return;document.getElementById('noti-title').innerText=activeMedia.title||"Web Media Playing";
...
function drawFakeVisualizer(){
 ...
 cvs.width=cvs.parentElement.clientWidth;cvs.height=14;const bufferLength=32;ctx.clearRect(0,0,cvs.width,cvs.height);const barW=(cvs.width/bufferLength)*2;let x=0;
 for(let i=0;i<bufferLength;i++){const barH=activeMedia&&!activeMedia.paused?(Math.random()*cvs.height):2;ctx.fillStyle=`#fff`;ctx.beginPath();ctx.roundRect(x,cvs.height-barH,barW-1.5,barH,2);ctx.fill();x+=barW;}
}drawFakeVisualizer();
